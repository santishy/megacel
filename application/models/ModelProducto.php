...
class ModelProducto extends CI_Model {
	...
	{
		$query=$this->db->query('call agregarCompra('.$data['id_compra'].','.$data['id_producto'].','.$data['cant'].','.$data['precio'].','.$data['id_color'].',@ban);');
		//$query->next_result();
		$query=$this->db->query('SELECT @ban');
		foreach ($query->result_array() as $row) 
		{
	...
	function getPrecio($id_precio,$id_producto)
	{
