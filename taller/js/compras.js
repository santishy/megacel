...
$(document).on('ready',function()
{
	  $('img').error(function(){
		...
		$(this).unbind("error").attr('src',"http://ventaliberacell.com/img/noimagen.gif");
		});
...
function insertarCarrito()
{
	...
	$.ajax({
		...
		beforeSend:function()
		{
			//$("#btnInsertarCarrito").attr('disabled",true);
		},
		type:'post',
	    ...
	    {
	    	//$("#btnInsertarCarrito").attr('disabled",false);
	    }
	})
}
...
function insertarPrecio()
{
	...
	$.ajax({
		...
		success:function(resp)
		{
			...
			if(typeof(resp.permiso) == "undefined")
			{
...
function insertarCarritoV()
{
	...
	$("#nombre_productoV").val($("#nombre_productoV").val().replace("/","-"));
	console.log($("#nombre_productoV").val()+" soy un rockanrolla");
	$.ajax({
		url:ruta,
		beforeSend:function()
		{
