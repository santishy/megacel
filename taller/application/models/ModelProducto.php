...
class ModelProducto extends CI_Model {
	...
	function getMarcas()
	{
		$query=$this->db->query('select *from marcas order by marca asc');
		return $query;
	}
	...
	{
		$query=$this->db->update('imagenes',$data);
		return $query;
	}
	function insertarImg($data)
	{
		$query=$this->db->insert('imagenes',$data);
		return $query;
	}
	function updateFondoFijo($data,$id){
		$this->db->where('id',$id);
		return $this->db->update('det_color',$data);
	}
	...
	{
		$query=$this->db->query('select *from categorias where status=1 order by categoria asc');
		return $query;
	}
	...
	function numRowsProductosCategoria($id_categoria)
	{
		$query=$this->db->query('select count(p.id_producto) as nume from productos p left join categorias c on p.id_categoria=c.id_categoria  left join marcas m on
		m.id_marca=p.id_marca  left join det_color dc on dc.id_producto=p.id_producto  left join colores clr on
		clr.id_color=dc.id_color where activacion=1 and p.id_categoria='.$id_categoria);
		$nume=0;
		foreach($query->result() as $row)
		{
			$nume=$row->nume;
	...
	function getProductosCategoria($id_categoria,$ini,$tope)
	{
		$query=$this->db->query('select p.id_producto,dc.id,p.nombre_producto,p.descripcion,(select distinct precios.id_precio from det_precios_ventas join precios on precios.id_precio=det_precios_ventas.id_precio where det_precios_ventas.id_producto=p.id_producto and tipo="publico" and activo=1 ) as precio, c.id_categoria,c.categoria,m.id_marca,m.marca,
		clr.color,clr.id_color,p.modelo,
		(select distinct precios.precio from det_precios_ventas join precios on precios.id_precio=det_precios_ventas.id_precio where det_precios_ventas.id_producto=p.id_producto and tipo="publico" and activo=1 ) as precio_publico,
		p.existencia,dc.exist from productos p left join categorias c on p.id_categoria=c.id_categoria  left join marcas m on
		m.id_marca=p.id_marca  left join det_color dc on dc.id_producto=p.id_producto  left join colores clr on
		clr.id_color=dc.id_color where activacion=1 and p.id_categoria='.$id_categoria.' limit '.$ini.','.$tope);
		return $query;
	}
	...
	{
		$query=$this->db->query('select p.id_precio,p.tipo,p.precio from productos pr join det_precios_ventas d on pr.id_producto=d.id_producto join precios p on d.id_precio=p.id_precio where d.id_producto='.$id.' and d.activo=1;');
		return $query;
	}
