...
class Venta extends CI_Controller
{
	...
	function crearNota()
	{
		...
		$permisos[2]=3;
		$this->funciones->logueado($permisos,$this->session->userdata('id_user'),$this->session->userdata('tipo'));
		return $this->nota($this->session->userdata('id_venta'));
	}
	function nota($id_venta=0){
		if(!$id_venta)
			$id_venta=$this->uri->segment(3);
		$query=$this->ModelVenta->getNota($id_venta);
		$row=$query->row_array();
		...
		$pdf->Ln(10);
		$pdf->Cell(0,5,'SUPERCEL S.A DE C.V.',0,1,'C');
		$pdf->SetFont('Arial','',8);
		$pdf->Cell(0,5,'RFC EAEA9405227P2',0,1,'C');
		$pdf->Cell(0,1,'CONSITUCION #45',0,1,'C');
		$pdf->Cell(0,5,'TEL 53 2 7572',0,1,'C');
		$pdf->SetFont('Arial','',7);
		...
		$pdf->SetFont('Arial','B',10);
		$pdf->SetFont('Arial','',8);
		$query=$this->ModelVenta->getVenta($id_venta);
		$pdf->SetFont('Arial','B',9);
		$pdf->SetTextColor(255,255,255);
