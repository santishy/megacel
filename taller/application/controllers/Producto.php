...
class Producto extends CI_Controller
{
	...
	public function updateFondoFijo(){
		$num=is_numeric($this->input->post('value'));
		$json['success']=false;
		if($num)
		{
			$data[$this->input->post('name')]=$this->input->post('value');
			$query=$this->ModelProducto->updateFondoFijo($data,$this->input->post('pk'));
			$json['success']=$query;
		}
	...
	function buscarCategoria()
	{
		...
		redirect('Producto/listProductoCategoria');
	}
	function listProductoCategoria()
	{
		$permisos[0]=1;
		$permisos[1]=2;
		$permisos[2]=3;
		...
		if(empty($offset))
			$offset=0;
		$config['base_url']=base_url().'producto/listProductoCategoria';
		$config['total_rows']=$this->ModelProducto->numRowsProductosCategoria($this->session->userdata('id_categoria'));
		$config['per_page']=50;
		$connfig['num_links']=5;
		...
		$data['productos']=$this->ModelProducto->getProductosCategoria($this->session->userdata('id_categoria'),$offset,$config['per_page']);
		date_default_timezone_set('America/Monterrey');
	...
	function listProducto($ban)
	{
		...
		if($this->session->userdata('clave'))
			$config['total_rows']=$this->ModelProducto->numRowsProductos($this->session->userdata('clave'));
		else
	...
	function guardarFoto()
	{
		...
		$dbimg['id']=$this->input->post('id_producto');
		$img = str_replace('data:image/png;base64,', '', $img);
		$img = str_replace(' ', '+', $img);
		$data = base64_decode($img);
		$im = imagecreatefromstring($data);  //convertir text a imagen
		$query=$this->ModelProducto->getImg($dbimg['id']);
		if($query->num_rows()>0){
			$vec['query']=$this->ModelProducto->actualizarImg($dbimg);
		}
		else
			$vec['query']=$this->ModelProducto->insertarImg($dbimg);
		if ($im !== false) {
		    imagejpeg($im, './img/'.$dbimg['id'].'.jpg'); //guardar a server
