	  ...
	  <div class="panel-body">
	  	...
	  	<div class="row">
	  		<div class="col-md-7">
				...
				<form class="" id="frmCategoria" action="<?=base_url()?>Producto/buscarCategoria" method="post">
					<div class="form-group" style="border:4px solid #944dff;margin-top:10px;padding:15px">
						<label>Categoria</label>
						<select name="id_categoria" class="form-control" id="search_categoria">
	  ...
	  <table class="table tabla table-striped table-hover" id="tabla-productos">
	  	...
	  	<tbody>
	  		<form action="<?=base_url()?>producto/agregarPrecioVenta" id="frmChecks">
		  		...
		  		{?>
		  		<tr>
		  			<td><img class="img-responsive" STYLE="width:100%"src="<?=base_url().'img/'.$row->id.'.jpg'?>">
		  			<td><?=$row->id?></td>
					...
					<td style="background-color:#e6fff2;text-align:center;font-size:bold">
						<a href="#"
							data-pk='<?=$row->id_producto?>'
							data-url='<?=base_url()?>Precio/update'
							data-value="<?=$row->precio_publico?>"
							data-name="precio"
							data-title='Precio Publico'
							data-type="number"
...
<script>
	...
	function init()
	{
		...
		$(".btnPic").on('click',function(){
			$("#modalFoto").modal('show');activarVideo()
			idfoto=id=$(this).parent().parent().data('idc');
		});
	...
	function guardarFoto()
	{
		...
		$.ajax({
			...
			type:'post',
			data:{dataImg:imgData,id_producto:idfoto},
			dataType:'json',
			success:function(resp)
			{
				console.log(resp.ban)
